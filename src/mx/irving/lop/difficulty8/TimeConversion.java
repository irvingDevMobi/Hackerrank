...
public class TimeConversion {
    ...
    public TimeConversion(String input) {
        mInput = input;
    }

    public String get24HourFormat() {
        int end = mInput.length() -2;
        char indicator = mInput.charAt(end);
        int hour = getHour();
        if (indicator == 'P') {
            if (hour != 12) {
                hour += 12;
            }
            return hour + mInput.substring(2, end);
        }
        else {
            if (hour == 12) {
                return "00" + mInput.substring(2, end);
            } else {
                return mInput.substring(0, end);
            }
        }
    }

    private int getHour() {
        return Integer.parseInt(mInput.substring(0,2));
    }
}
